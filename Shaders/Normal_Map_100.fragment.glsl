...
DiffAndSpecLightColors GetDiffuseAndSpecularLightValues( Light light, vec2 texCoords )
{
    ...
    {
        normalTexel = vec4( 0.5, 0.5, 1.0, 1.0 );
    }
    ...
    else
    {
        ...
        specularTexel = vec4( 1.0, 1.0, 1.0, 1.0 );
    }
	...
	float diffuseCoefficient = Clamp( dot( normalWorldSpace, vec3( 0.0, sin(0.785398163), cos(0.785398163) ) ), 0.0, 1.0 );
    ...
    lightColors.m_diffuseLightColor = vec4( 1.0, 1.0, 1.0, 1.0 );
    lightColors.m_specularLightColor = vec4( 1.0, 1.0, 1.0, 1.0 );
...
void main()
{
	...
	DiffAndSpecLightColors lightColors = GetDiffuseAndSpecularLightValues( u_light, adjustedTexCoords );
	float lightStrength = GetLocalLightStrengthPercentage( u_light );
    ...
    {
        emissiveTexel = vec4( 0.0, 0.0, 0.0, 1.0 );
    }
    else
    {
    ...
    if( u_isUsingDiffuse > 0 )
    {
        diffuseTexel = texture2D( u_diffuseTexture, a_textureCoordinates );
    }
    else
    {
        
        diffuseTexel = vec4( 0.0, 0.0, 0.0, 1.0 );
    }
    ...
    if( u_isUsingNormal > 0 )
    {
        ...
        normalTexel = texture2D( u_normalTexture, a_textureCoordinates );
    }
    else
    {
        normalTexel = vec4( 1.0, 1.0, 1.0, 1.0 );
    }
    
    vec4 specularTexel;
    if( u_isUsingSpecular > 0 )
    {
        specularTexel = texture2D( u_specularTexture, a_textureCoordinates );
    }
    else
    {
        
        specularTexel = vec4( 0.0, 0.0, 0.0, 1.0 );
    }
    ...
    {
        emissiveTexel = texture2D( u_emissiveTexture, a_textureCoordinates );
    }
    
    gl_FragColor = ( ( diffuseTexel + emissiveTexel ) * normalTexel ) + specularTexel;
}